 ...
 *
 * @param matches  flat list of { startsAt, upcoming } from the parsed page
...
export function pollPlan(matches, nowMs) {
  const timed = matches
    .filter((m) => m.startsAt)
    .map((m) => ({ at: Date.parse(m.startsAt), upcoming: m.upcoming }))
  ...
  // Live: started, not yet reported finished, and not so long ago that the
  // page has clearly stopped being updated for it.
  const live = timed.filter((m) => m.upcoming === false ? false : m.at <= nowMs && nowMs - m.at < ASSUME_LIVE);
  if (live.length) {
    return { state: "live", everyMs: LIVE, reason: `${live.length} match${live.length === 1 ? "" : "es"} under way`, nextMatchAt: null };
  }

  const next = timed.find((m) => m.at > nowMs && m.upcoming !== false);
  if (next) {
